...
class Inputs:
    def __init__(self):
        self.env=data_xlsx(ROOT/'A题/附件/附件1.xlsx',3)
        self.rad=data_xlsx(ROOT/'A题/附件/附件2.xlsx',2)
        self.rad[:,1]*=.01
...
def run(case,nr,nz,end=None,rtol=2e-9,max_step=240.,shrink=True,end_transfer=True,flux='integral'):
    inp=Inputs();op=GeometryFV(nr,nz,case,inp,shrink=shrink,end_transfer=end_transfer,flux=flux)
    ...
    if (DEST/(label+'.json')).exists():raise FileExistsError(label)
...
if __name__=='__main__':
    ...
    a=p.parse_args();run(a.case,a.nr,a.nz,a.end,a.rtol,a.max_step,not a.fixed,not a.sealed_ends,a.flux)
